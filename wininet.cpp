...
static void AllowSelfSignedCertificates(HINTERNET hInet)
{
	DWORD dwSecFlags;
	DWORD dwBuffLen = sizeof(dwSecFlags);

	CWA(wininet, InternetQueryOptionA)(hInet, INTERNET_OPTION_SECURITY_FLAGS, (LPVOID)&dwSecFlags, &dwBuffLen);

	dwSecFlags |= SECURITY_FLAG_IGNORE_UNKNOWN_CA | SECURITY_FLAG_IGNORE_CERT_CN_INVALID | SECURITY_FLAG_IGNORE_CERT_DATE_INVALID;
	CWA(wininet, InternetSetOption)(hInet, INTERNET_OPTION_SECURITY_FLAGS, &dwSecFlags, sizeof(dwSecFlags));
}
...
#else
	HINTERNET hInet = CWA(wininet, InternetOpenA)(pstrUserAgent,
		dwFlags & WICF_USE_IE_PROXY ? INTERNET_OPEN_TYPE_PRECONFIG : INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
#endif
	if(hInet == NULL)
		return NULL;
...
void Wininet::_CloseConnection(HINTERNET hConnect)
{
	...
	if(r && hInet && dwSize == sizeof(HINTERNET))
		CWA(wininet, InternetCloseHandle)(hInet);
#if(BO_DEBUG > 0 && defined(WDEBUG1))
	else
		WDEBUG0(WDDT_ERROR, "Opps! Parent no founded for hConnection!");
#endif
}
...
bool Wininet::callCallback(HINTERNET handle, DWORD_PTR context, DWORD internetStatus, LPVOID statusInformation, DWORD statusInformationLength)
{
	...
	for(; handle != NULL;)
	{
		...
		if(CWA(wininet, InternetQueryOptionA)(handle, INTERNET_OPTION_CALLBACK, &isc, &size) == TRUE && 
			isc != INTERNET_NO_CALLBACK && isc != INTERNET_INVALID_STATUS_CALLBACK)
		{
#if defined WDEBUG1
		...
		size = sizeof(HINTERNET);
		if(CWA(wininet, InternetQueryOptionA)(handle, INTERNET_OPTION_PARENT_HANDLE, &handle, &size) == FALSE)
			break;
	}
	return false;
}
...
HINTERNET Wininet::_SendRequest(HINTERNET hConnect, LPSTR pstrURI, LPSTR pstrReferer, void *pPostData, DWORD dwPostDataSize, DWORD dwFlags)
{
	...
	if(dwFlags & WISRF_KEEP_CONNECTION)
		dwReqFlags |= INTERNET_FLAG_KEEP_CONNECTION;
	if(dwFlags & WISRF_IS_HTTPS)
	...
	if(hReq != NULL)
	{
		...
		bool bAllowingInvalidCAs = false;
		Retry:
		if(CWA(wininet, HttpSendRequestA)(hReq, headers, headersSize, pPostData, dwPostDataSize))
		{
			DWORD dwStatus = 0, dwSize = sizeof(DWORD);
			if(CWA(wininet, HttpQueryInfoA)(hReq, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER, &dwStatus, &dwSize, NULL) && 
				dwStatus == HTTP_STATUS_OK)
				return hReq;
		}
		else if(!bAllowingInvalidCAs)
		{
			AllowSelfSignedCertificates(hReq);
			bAllowingInvalidCAs = true;
			goto Retry;
		}
...
{
	if(dwSizeLimit == 0 || dwSizeLimit > WININET_MAXBYTES_TO_MEM)
	...
	if(HttpTools::_parseUrl(pcud->pstrURL, &ud))
	{
		...
		for(BYTE bi = 0; bi < pcud->bTryCount; bi++)
		{
			...
			if(bi > 0)
			{
				...
				{
					if(CWA(kernel32, WaitForSingleObject)(pcud->hStopEvent, pcud->dwRetryDelay) != WAIT_TIMEOUT)
						goto END;
				}
				else 
					CWA(kernel32, Sleep)(pcud->dwRetryDelay);
			}
			...
			for(BYTE pp = 0; pp < pp_m; pp++)
			{
				...
				if(hConnect)
				{
					...
					if(hRequest)
					{
						if(pcud->DownloadData_pstrFileName)
							r = _DownloadDataToFile(hRequest, pcud->DownloadData_pstrFileName, pcud->DownloadData_dwSizeLimit, pcud->hStopEvent);
						else 
							r = _DownloadData(hRequest, pBuf, pcud->DownloadData_dwSizeLimit, pcud->hStopEvent);
...
void *Wininet::_queryOptionExA(HINTERNET internet, DWORD option, LPDWORD lenght)
{
	...
	*lenght = 0;
	...
	if(CWA(wininet, InternetQueryOptionA)(internet, option, nullptr, lenght) == FALSE && (*lenght) > 0 && CWA(kernel32, GetLastError)() == ERROR_INSUFFICIENT_BUFFER)
	{
		if((buf = Mem::alloc(*lenght)) != nullptr)
		{
			if(CWA(wininet, InternetQueryOptionA)(internet, option, buf, lenght) == TRUE)
				return buf;
			Mem::free(buf);
		}
	}
...
void *Wininet::_queryOptionExW(HINTERNET internet, DWORD option, LPDWORD lenght)
{
	...
	*lenght = 0;
	...
	if(CWA(wininet, InternetQueryOptionW)(internet, option, NULL, lenght) == FALSE && (*lenght) > 0 && 
		CWA(kernel32, GetLastError)() == ERROR_INSUFFICIENT_BUFFER)
	{
		if((buf = Mem::alloc(*lenght)) != NULL)
		{
			if(CWA(wininet, InternetQueryOptionW)(internet, option, buf, lenght) == TRUE)
				return buf;
			Mem::free(buf);
		}
	}
...
LPSTR Wininet::_queryInfoExA(HINTERNET request, DWORD infoLevel, LPDWORD lenght, LPDWORD index)
{
	...
	*lenght = 0;
	...
	if((infoLevel & HTTP_QUERY_FLAG_NUMBER) == 0 &&
		CWA(wininet, HttpQueryInfoA)(request, infoLevel, tmpBuf, lenght, index) == FALSE && 
		CWA(kernel32, GetLastError)() == ERROR_INSUFFICIENT_BUFFER)
	{
		if((buf = (LPSTR)Mem::alloc(*lenght)) != NULL)
		{
			if(CWA(wininet, HttpQueryInfoA)(request, infoLevel, buf, lenght, index) == TRUE)
				return buf;
			Mem::free(buf);
		}
	}
