	...
	while(oldOffset < size)
	{
		...
		else if(string[oldOffset] == '<')
		{
			if(level++ == 0)
			{
				...
				{
					if(curSize > TAG_SCRIPT_SIZE && _compareTagNameA(TAG_SCRIPT, TAG_SCRIPT_SIZE, curPos))
						isScript = true;
		...
		{
			if(string[oldOffset] == '&' && (size - oldOffset) > SYMB_SPACE_SIZE &&  
				CWA(shlwapi, StrCmpNIA)(string + oldOffset + 1, SYMB_SPACE, SYMB_SPACE_SIZE) == 0)
			{
				string[newOffset] = ' ';
	...
	*size = 0;
	...
	while(data < dataEnd)
	{
			...
			(DWORD_PTR)header == GMH_REQUEST_HTTP_VERSION))
		{
			...
			{
				if((DWORD_PTR)header == GMH_HTTP_METHOD)
					dataEnd = spaces[0];
				else if((DWORD_PTR)header == GMH_HTTP_URI)
				{
					cur = spaces[0] + 1; 
					dataEnd = spaces[1];
				}
				else if((DWORD_PTR)header == GMH_REQUEST_HTTP_VERSION)
				{
					cur = spaces[1] + 1;
				}
