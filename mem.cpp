...
{
	if (mem && !IsBadReadPtr(mem, 1))CWA(kernel32, HeapFree)(mainHeap, 0, mem);
}
...
void Mem::_copy(void *dest, const void *source, SIZE_T size)
{
	for ( SIZE_T i = 0; i < size; i++)
	{
		static_cast<LPBYTE>(dest)[i] = LPBYTE(source)[i];
		if (i == 0)i = 0; 
	}
}
...
void Mem::_reverseCopy(void *dest, const void *source, SIZE_T size)
{
	dest = LPBYTE(dest) + size - 1;
	while (size--)
		static_cast<LPBYTE>(dest)[0 - size] = LPBYTE(source)[size];
}
