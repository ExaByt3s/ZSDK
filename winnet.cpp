...
HINTERNET Wininet::_Connect(LPSTR pstrUserAgent, LPSTR pstrHost, WORD wPort, DWORD dwFlags)
{
  ...
  for(DWORD i = 0; i < sizeof(WinInetOptions) / sizeof(WININETOPTION); i++)
  	CWA(wininet, InternetSetOptionA)(hInet, WinInetOptions[i].dwOption, static_cast<void *>(&WinInetOptions[i].dwValue), sizeof(DWORD));

  //Подключение, как таковое соединение не устанавливается. Ебанутый M$.
  HINTERNET hConnect = CWA(wininet, InternetConnectA)(hInet, pstrHost, wPort, nullptr, nullptr, INTERNET_SERVICE_HTTP, 0, NULL);
  if(hConnect == nullptr)
  {
